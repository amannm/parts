    ...
    def build_bcs(
        ...
        for bc in config.get("bcs", []):
            ...
            if bc["type"] == "dirichlet":
                ...
                L_terms.append(axis_weight * q * v * ds(tag_id))
            elif bc["type"] == "robin":
                ...
                L_terms.append(axis_weight * alpha * t_inf * v * ds(tag_id))
            else:
        ...
        if isinstance(sources, dict) and isinstance(facet_map, dict):
            for group in ("surface", "line"):
                for spec in sources.get(group, []) or []:
                    ...
                    if tag_id is None:
                        raise KeyError(f"Unknown facet tag for transient heat source: {tag_name}")
